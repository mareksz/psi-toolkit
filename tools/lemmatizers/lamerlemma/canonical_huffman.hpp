#ifndef CANONICAL_HUFFMAN_HDR
#define CANONICAL_HUFFMAN_HDR
...
class CanonicalHuffman
{
  ...
  public:
    ...
    template <class BitWrapper>
    boost::optional<Data> Read(BitWrapper& bitWrapper)
    {
      if(bitWrapper.TellFromEnd())
      {
        ...
        return boost::optional<Data>(
            m_symbols[m_lengthIndex[len] + (intCode - m_firstCodes[len])]
        );
      }   
      return boost::optional<Data>();
    }
    ...
    uint64_t Save(std::FILE* pFile)
    {
      ...
      std::fwrite(&size, sizeof(uint64_t), 1, pFile);
      std::fwrite(&m_symbols[0], sizeof(Data), size, pFile);
      
      size = m_firstCodes.size();
      std::fwrite(&size, sizeof(uint64_t), 1, pFile);
      std::fwrite(&m_firstCodes[0], sizeof(uint64_t), size, pFile);
      
      size = m_lengthIndex.size();
      std::fwrite(&size, sizeof(uint64_t), 1, pFile);
      std::fwrite(&m_lengthIndex[0], sizeof(uint64_t), size, pFile);
