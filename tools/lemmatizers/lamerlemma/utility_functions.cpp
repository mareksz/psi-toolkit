...
std::string parseInput(const std::string& input, bool parseTag, bool parseMorpho)
{
...
void parseResult(const std::string& result, LemmaMap &lemmaMap,
    ...
    if(parseTag || parseMorpho) {
        ...
        if(parseMorpho && fields.size() == 4) {
            ...
            boost::split(morphofields, fields[3], boost::is_any_of(LEXICON_SUBFIELD_SEPARATOR));
