...
Annotator* Inflector::Factory::doCreateAnnotator(
    ...
    Inflector* tagger = new Inflector(
            lang,
            options.count("train"),
...
void Inflector::Factory::doAddLanguageIndependentOptionsHandled(
...
std::string Inflector::Factory::doGetName() const {
    return "inflector";
}

std::string Inflector::Factory::doGetSubType() const {
    return "tagger";
}

boost::filesystem::path Inflector::Factory::doGetFile() const {
    return __FILE__;
}

std::list<std::list<std::string> > Inflector::Factory::doRequiredLayerTags() {
    return std::list<std::list<std::string> >();
}

std::list<std::list<std::string> > Inflector::Factory::doOptionalLayerTags() {
    return std::list<std::list<std::string> >();
}

std::list<std::string> Inflector::Factory::doProvidedLayerTags() {
    return std::list<std::string>();
}

const std::string Inflector::Factory::DEFAULT_MODEL_FILE
= "%ITSDATA%/%LANG%.blm";
const int Inflector::Factory::DEFAULT_ITERATIONS = 50;
const std::string Inflector::Factory::DEFAULT_UNKNOWN_POS_LABEL = "ign";
const std::string Inflector::Factory::DEFAULT_CARDINAL_NUMBER_POS_LABEL = "card";
const std::string Inflector::Factory::DEFAULT_PROPER_NOUN_POS_LABEL = "name";

LatticeWorker* Inflector::doCreateLatticeWorker(Lattice& lattice) {
    return new Worker(*this, lattice);
}

Inflector::Worker::Worker(Inflector& processor, Lattice& lattice):
    LatticeWorker(lattice), processor_(processor) {
    }

void Inflector::Worker::doRun() {
...
std::string Inflector::doInfo() {
    return "maximum entropy inflector";
}

Inflector::Inflector(const std::string& langCode, bool trainMode_, std::string modelFile_,
...
bool Inflector::training() {
    return trainMode;
}

std::string Inflector::getModelFile() {
    return modelFile;
}

void Inflector::saveModel(std::string path) {
    m.save(path, !saveTextFile);
}

void Inflector::loadModel(std::string path) {
    m.load(path);
    posModelLoaded = true;
}

void Inflector::tag(Lattice &lattice) {
    if (!posModelLoaded) {
...
void Inflector::tagSegment(Lattice &lattice, TokenEdgesMap tokenEdgesMap) {
    std::vector<Outcome> tags = getTokenTags(lattice, tokenEdgesMap);
    applyTokenTags(lattice, tokenEdgesMap, tags);
}

void Inflector::train(Lattice &lattice) {
...
void Inflector::addSampleSentences(Lattice &lattice) {
    if (! openForEvents) {
        m.begin_add_event();
        openForEvents = true;
    }
...
void Inflector::addSampleSegment(Lattice &lattice,
...
Inflector::TokenEdgesMap Inflector::createTokenEdgesMap(
...
Inflector::Context Inflector::createContext(Lattice &lattice,
        TokenEdgesMap tokenEdgesMap,
...
std::vector<Inflector::Outcome> Inflector::getTokenTags(Lattice &lattice,
        TokenEdgesMap tokenEdgesMap) {
...
Inflector::Outcome Inflector::getBestTag(Lattice &lattice,
        Lattice::EdgeDescriptor token, Inflector::Context context) {
    LayerTagMask formMask = lattice.getLayerTagManager().getMaskWithLangCode(
        "form", langCode_);
...
std::string Inflector::getPrevTag(Lattice &lattice, TokenEdgesMap tokenEdgesMap,
        int tokenIndex) {
    LayerTagMask formMask = lattice.getLayerTagManager().getMaskWithLangCode(
        "form", langCode_);
    std::string prevTag = "";
...
void Inflector::addCurrentTag(Lattice &lattice, Lattice::EdgeDescriptor token,
        Inflector::Context context) {
    LayerTagMask formMask = lattice.getLayerTagManager().getMaskWithLangCode(
        "form", langCode_);
    Lattice::VertexDescriptor currentVertex =
        lattice.getEdgeSource(token);
    ...
    while (formIt.hasNext()) {
        ...
        //std::cerr << "class: " << currentTag << std::endl;
        m.add_event(context, currentTag);
        break;
    }
}

void Inflector::applyTokenTags(Lattice &lattice, TokenEdgesMap tokenEdgesMap,
        std::vector<Inflector::Outcome> tags) {
    LayerTagMask formMask = lattice.getLayerTagManager().getMaskWithLangCode(
        "form", langCode_);
    size_t i = 0;
    while (i < tags.size()) {
...
bool Inflector::formMatchesTag(Lattice &lattice,
        Lattice::EdgeDescriptor form, Inflector::Outcome tag) {
...
std::string Inflector::getFormLemma(Lattice &lattice,
        Lattice::EdgeDescriptor edge) {
    
    return lattice.getAnnotationText(edge);
}

std::string Inflector::getFormPartOfSpeech(Lattice &lattice,
        Lattice::EdgeDescriptor edge) {
    
    return lattice.getEdgeAnnotationItem(edge).getCategory();
...
std::string Inflector::getFormMorphoTag(Lattice &lattice,
        Lattice::EdgeDescriptor edge) {
...
std::string Inflector::getPartOfSpeechFromMorphoTag(std::string tag) {
...
std::list<std::pair<std::string, std::string> >
    Inflector::getAttributesFromMorphoTag(std::string tag) {
    std::list<std::pair<std::string, std::string> > attributes;
    std::string tempTag = "";
    if (tag.find(",") > 0) {
...
bool Inflector::hasLexemeEdgeMatchingTag(Lattice &lattice,
...
Lattice::EdgeDescriptor Inflector::getLexemeEdgeMatchingTag(Lattice &lattice,
        Lattice::EdgeDescriptor token, std::string tag) {
    LayerTagMask lexemeMask = lattice.getLayerTagManager().getMaskWithLangCode(
        "lexeme", langCode_);
    Lattice::VertexDescriptor vertex =
...
bool Inflector::lexemeEdgeMatchesTag(Lattice &lattice,
...
bool Inflector::lemmaEdgeExists(Lattice &lattice,
...
void Inflector::addLemmaEdge(Lattice &lattice,
        Lattice::EdgeDescriptor token, std::string lemma) {
...
bool Inflector::lexemeEdgeExists(Lattice &lattice,
        Lattice::EdgeDescriptor token, std::string lemma,
...
Lattice::EdgeDescriptor Inflector::addLexemeEdge(Lattice &lattice,
        Lattice::EdgeDescriptor token, std::string lemma,
        std::string partOfSpeech) {
    LayerTagMask lemmaMask = lattice.getLayerTagManager().getMaskWithLangCode(
        "lemma", langCode_);
...
void Inflector::addFormEdge(Lattice &lattice,
...
bool Inflector::isDiscarded(Lattice &lattice, Lattice::EdgeDescriptor edge) {
