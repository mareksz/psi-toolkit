...
find . -regextype posix-extended -regex '.*\.(cpp|hpp|h|c)' ! -regex '\./(build|server/mpfd-parser|utf8|maxent|sundown|fex|tiniconv|uchardet)/.*' ! -path './common/config.hpp' | vera++ - -profile psi -showrules 2>&1 | ./vera++2cppcheck.pl > vera++-result.xml
