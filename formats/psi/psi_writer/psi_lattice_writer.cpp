...
void PsiLatticeWriter::Worker::doRun() {
    ...
    while (ei.hasNext()) {
        ...
        if (edgeOrdinalMap.count(edge)) {
            continue;
        }
        if (lattice_.isEdgeHidden(edge)) {
            continue;
        }
        printEdge(edge, quoter, edgeOrdinalMap, ordinal, latticeTextCovered);
    }
...
void PsiLatticeWriter::Worker::printEdge(
